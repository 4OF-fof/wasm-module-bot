...
Modulebot は、信頼しない Rust/WASM plugin を実行する Discord bot host です。
...
| effect 実行              | Discord reply、message send、HTTP fetch、LLM agent、channel history を host 側で実行する。                                                    |
...
`host/data/modulebot.sqlite` です。`MODULEBOT_DATA_DIR` を設定すると、その directory に
`modulebot.sqlite` を作成します。
...
通常の plugin 開発者は ABI を直接実装しません。`modulebot-plugin-api` crate の
...
| `discord.message.send`      | `discord.message.send`      | 実行 target が text channel であること。                                 |
| `http.fetch`                | `http.get`                  | method が `GET` で、URL hostname が origin policy で許可されていること。 |
| `agent`                     | `agent`                     | セッション管理（TTL, リングバッファ）のもと LLM 呼び出しを許可。         |
| `discord.channel.history`   | `discord.channel.history`   | `before` + `limit` によるページネーションでチャンネル履歴を取得。        |
